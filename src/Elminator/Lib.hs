...
type GenConfig = Map.Map MData ([GenOption], HType)
    ...
    wrapInParaConditionally tn =
        if includePara
            then case td of
                ...
                TExternal ei ->
                    if not $ List.null (exTypeArgs ei)
                        then wrapInPara tn
                        else tn
                _ -> tn
            else tn
...
getConstructorsFields nec =
    List.concat $ NE.toList $ NE.map getConstructorFields_ nec
