...
import Control.Monad.Reader as R (MonadReader (ask), zipWithM)
import Data.Aeson as Aeson (Options)
...
import Language.Haskell.TH (Con (NormalC, RecC), Type (AppT, VarT))
import Prelude
    ...
    let fieldCount = P.length exprs
     ...
     in if fieldCount < 9
            then decU8
                ...
                List.foldl' (\a v -> EFuncApp (EFuncApp "seqApp" a) v) decU8 $
                    List.drop 8 exprs
                    ...
                    case tds of
                        ...
                        (_ : _) ->
                            ELet [generateTupleDecoder idx tds] $
                                EName $
                                    T.concat ["decodeTuple", pack $ show idx]
